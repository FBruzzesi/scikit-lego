...
class WithoutLiersCV(BaseCrossValidator):
    r"""Cross-validation splitter that filters out data points labeled as anomalies during the splitting process
    to exclude them from the training sets.

    The anomaly label is specified by the `anomalous_label` parameter. Data points with this label are excluded
    from the training sets, while the test sets are yielded exactly as generated by the base `cv` splitter:
    anomalous points are dropped from the training folds, not moved to the test folds.

    Hence an anomalous point appears in a test set only if the base splitter assigned it there.
    For splitters that partition the data, such as `KFold`, every anomalous point still appears in exactly one
    test set; for splitters that do not, such as `ShuffleSplit`, it may appear in none.

    Set `exclude_from_test=True` to exclude anomalous points from the test sets as well.

    Parameters
    ----------
    cv : BaseCrossValidator
        The base cross-validation splitter used to split the data. It must have a `split` method that yields
    ...
    exclude_from_test : bool, default=False
        Whether to exclude anomalous data points from the test sets as well.
        If `False`, the test sets are yielded exactly as generated by the base `cv` splitter.
    ...
    def __init__(self, cv: BaseCrossValidator, anomalous_label: int = -1, *, exclude_from_test: bool = False) -> None:
        self.cv = cv
    ...
    def split(
        ...
        anomalous label from the training set (and from the test set if `exclude_from_test=True`).
        ...
        train_index : np.ndarray
            Indices of the training set, excluding anomalous data points.
        test_index : np.ndarray
            Indices of the test set as generated by the base `cv` splitter, excluding anomalous data points
            only if `exclude_from_test=True`.
        """
        inliers = np.asarray(y) != self.anomalous_label
        for train_index, test_index in self.cv.split(X, y, groups):
            train_index = train_index[inliers[train_index]]
            if self.exclude_from_test:
                test_index = test_index[inliers[test_index]]
            yield train_index, test_index

    def get_n_splits(
        self, X: npt.ArrayLike | None = None, y: npt.ArrayLike | None = None, groups: npt.ArrayLike | None = None
    ) -> int:
